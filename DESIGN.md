...
The visual style reads like a quietly editorial print magazine. The base canvas is off-white `{colors.canvas}` (#f5f5f5) holding warm near-black ink `{colors.ink}` (#0c0a09). The visual emphasis is **photographic, not chromatic**: soft pastel atmospheric gradient orbs (mint, peach, lavender, sky, rose) drift through the page as the only "color" moments. There is no neon accent, no saturated CTA color, no dark-canvas dev-tools atmosphere.
...
Waldenburg is licensed. Open-source substitute: **EB Garamond** at weight 300 (slightly more humanist) or **GT Sectra** (closer to Waldenburg's modernity). Use Inter directly for body.
...
**`top-nav`** — Background `{colors.canvas}`, text `{colors.ink}`, height 64px. Layout: product title left, primary navigation in the center, account controls and the primary CTA right.
